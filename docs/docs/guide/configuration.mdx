...
from hs_net import NetConfig, EngineEnum, DEFAULT_USER_AGENT
...
config = NetConfig(
    ...
    user_agent=DEFAULT_USER_AGENT,  # User-Agent，默认当前版桌面 Chrome
...
默认是当前版本的桌面 Chrome，无需配置。要换用内置的真实 UA 池，传快捷方式即可（不区分大小写）：

```python
net = SyncNet(user_agent="random")      # 任意桌面浏览器
net = SyncNet(user_agent="chrome")      # 指定浏览器
net = SyncNet(user_agent="mobile")      # 任意移动端浏览器
net = SyncNet(user_agent="android")     # 指定移动平台
net = SyncNet(user_agent="bot")         # 任意搜索引擎爬虫
net = SyncNet(user_agent="baiduspider") # 指定爬虫
net = SyncNet(user_agent="wechat")      # 微信内置浏览器
net = SyncNet(user_agent="MyBot/1.0")   # 自定义 UA，原样使用
```
...
UA 数据内置在包里（约 13 KB，无额外依赖），由 `scripts/gen_ua_data.py` 从上游源合并生成，刷新只需重跑该脚本。
...
```python title="team_config.py"
from hs_net import NetConfig, EngineEnum, DEFAULT_USER_AGENT, Net, SyncNet
